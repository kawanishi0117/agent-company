/**
 * AI Adapter Fallback プロパティテスト
 *
 * Property 15: AI Adapter Fallback
 * - プライマリアダプタが失敗した場合、システムは設定されたフォールバックアダプタを
 *   使用してから失敗を報告すること
 *
...
/**
 * ChatWithToolsOptionsを生成するArbitrary
 */
...
describe('Property 15: AI Adapter Fallback', () => {
  ...
  /**
   * Property 15.1: プライマリアダプタが失敗した場合、フォールバックアダプタが使用される
   *
  ...
  /**
   * Property 15.2: プライマリアダプタが失敗した場合、フォールバックアダプタが使用される（chat）
   *
   * **Validates: Requirements 7.5**
   */
  it('Property 15.2: プライマリアダプタが失敗した場合、フォールバックアダプタが使用される（chat）', async () => {
    await fc.assert(
  ...
  /**
   * Property 15.3: プライマリアダプタが失敗した場合、フォールバックアダプタが使用される（chatWithTools）
   *
   * **Validates: Requirements 7.5**
   */
  it('Property 15.3: プライマリアダプタが失敗した場合、フォールバックアダプタが使用される（chatWithTools）', async () => {
    await fc.assert(
      fc.asyncProperty(chatWithToolsOptionsArb, async (options) => {
        // フォールバックアダプタでツール付きチャットを実行
  ...
  /**
   * Property 15.4: プライマリとフォールバック両方が失敗した場合、AdapterFallbackErrorが投げられる
   *
   * **Validates: Requirements 7.5**
   */
  it('Property 15.4: プライマリとフォールバック両方が失敗した場合、AdapterFallbackErrorが投げられる', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(generateOptionsArb, async (options) => {
        // 両方失敗する場合、AdapterFallbackErrorが投げられる
  ...
  /**
   * Property 15.5: プライマリアダプタが成功した場合、フォールバックは使用されない
   *
   * **Validates: Requirements 7.5**
   */
  it('Property 15.5: プライマリアダプタが成功した場合、フォールバックは使用されない', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(generateOptionsArb, async (options) => {
        ...
        // 検証: プライマリアダプタが使用されたこと
        expect(result.content).toContain('primary');

        // 検証: フォールバックアダプタは呼び出されていないこと
  ...
  /**
   * Property 15.6: リトライ回数が設定通りに動作する
   *
   * **Validates: Requirements 7.5**
   */
  it('Property 15.6: リトライ回数が設定通りに動作する', async () => {
    await fc.assert(
      fc.asyncProperty(maxRetriesArb, generateOptionsArb, async (maxRetries, options) => {
        ...
        // 検証: プライマリアダプタがmaxRetries回試行されたこと
  ...
  /**
   * Property 15.7: 間欠的な失敗からリトライで回復できる
   *
   * **Validates: Requirements 7.5**
   */
  it('Property 15.7: 間欠的な失敗からリトライで回復できる', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 1, max: 2 }), // 1-2回失敗してから成功
        generateOptionsArb,
        async (failuresBeforeSuccess, options) => {
          ...
          // 検証: プライマリアダプタが最終的に成功したこと
          expect(result.content).toContain('attempts');

          // 検証: フォールバックは使用されていないこと
          expect(successFallback.getCallCount()).toBe(0);
        }
      ),
      { numRuns: 30 }
...
describe('AdapterRegistry', () => {
  ...
  /**
   * 存在しないアダプタを取得しようとするとエラー
   */
  it('存在しないアダプタを取得しようとするとエラー', () => {
...
describe('createAdapterFromConfig', () => {
  ...
  /**
   * プライマリのみの設定でアダプタを作成できる
   */
  it('プライマリのみの設定でアダプタを作成できる', () => {
    const adapter = createAdapterFromConfig({ primary: 'test-primary' }, registry);
    expect(adapter.name).toBe('test-primary');
  });

  /**
   * フォールバック付きの設定でFallbackAdapterを作成できる
   */
  it('フォールバック付きの設定でFallbackAdapterを作成できる', () => {
...
describe('AI Adapter Fallback - Edge Cases', () => {
  /**
   * 空のメッセージ配列でもchatが動作する
   */
  it('空のメッセージ配列でもchatが動作する', async () => {
  ...
  /**
   * 空のツール配列でもchatWithToolsが動作する
   */
  it('空のツール配列でもchatWithToolsが動作する', async () => {
    const adapter = new SuccessAdapter('test');
   ...
   * 日本語プロンプトが正しく処理される
   */
  it('日本語プロンプトが正しく処理される', async () => {
    const adapter = new SuccessAdapter('test');
    const result = await adapter.generate({
      model: 'llama3',
      prompt: 'これは日本語のテストプロンプトです。絵文字も含む🚀',
    });
    expect(result.content).toContain('これは日本語のテストプロンプトです');
  });

  /**
   * 非常に長いプロンプトが処理される
   */
  it('非常に長いプロンプトが処理される', async () => {
    const adapter = new SuccessAdapter('test');
  ...
  /**
   * FallbackAdapterのisAvailableが正しく動作する
   */
  it('FallbackAdapterのisAvailableが正しく動作する', async () => {
    // プライマリが利用可能な場合
  ...
  /**
   * FallbackAdapterのsupportsToolsが正しく動作する
   */
  it('FallbackAdapterのsupportsToolsが正しく動作する', async () => {
  ...
  /**
   * FallbackAdapterのgetModelInfoが正しく動作する
   */
  it('FallbackAdapterのgetModelInfoが正しく動作する', async () => {
    const adapter = new FallbackAdapter(
      new SuccessAdapter('primary'),
      new SuccessAdapter('fallback'),
      { maxRetries: 1, retryDelayMs: 10 }
