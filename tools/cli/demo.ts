  ...
  try {
    const response = await adapter.chat({
      ...
      messages: [
        ...
        { role: 'assistant', content: 'AgentCompany is a framework for running AI agents as a company organization with governance and quality gates.' },
...
`;
  ...
  try {
    const response = await adapter.generate({
      ...
      system: 'You are a Quality Authority at AgentCompany. Review code for quality, readability, and best practices. Be concise.',
      temperature: 0.3,
      maxTokens: 400,
    });
...
async function main(): Promise<void> {
  ...
  log('  2. チケット実行: npx tsx tools/cli/agentcompany.ts run workflows/backlog/0001-sample.md', colors.reset);
